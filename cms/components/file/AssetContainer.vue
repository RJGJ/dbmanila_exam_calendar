<template lang="html">
  <validation-observer tag="div" class="group" :vid="`asset_form_${unique}`" :key="refresh">
    <validation-provider name="file" ref="provider" :vid="`asset_form_${unique}.${input_name}[]`" v-slot="{ errors, validate }" :rules="{ required: (required) ? true : false, image: (type == 'image') ? true : false, size: file_size, ext: extensions}" class="file">
    ...
    <div :class="[ 'preview_image', columns ]" v-if="files.length > 0">
      <div :class="[ 'preview', division, (type == 'file') ? 'big' : '' ]" v-for="(data, key) in populateFiles" :key="key">
        ...
        <template v-if="type == 'image'">
          ...
          <validation-provider tag="div" class="group bordered" name="alt" :vid="`asset_form_${key}.file_alt[]`" :rules="{ required: true, regex: '^[a-zA-Z0-9\_\-]*$', max: 50 }" v-slot="{ errors }">
