<template>
  <ValidationObserver tag="div" v-if="enabled" :vid="`image_form_${unique}`">
    <ValidationProvider tag="div" :class="[ 'group', (showTags) ? '' : ($parent.multiple) ? '' : 'nmb' ]" name="file" :vid="`image_form_${unique}.${input_name}[]`" ref="provider" v-slot="{ errors, validate }" :rules="file_rules">
      <input type="file" class="action_image" accept=".jpeg, .jpg, .png, .webp, .svg" :id="`${input_name}${unique}`" :name="`${input_name}[]`" ref="file" @change="getFile($event)">
      <label class="action_image_label" :for="`${input_name}${unique}`">{{ label }}</label>
    ...
    <div class="form_tags_group" v-if="showTags">
      ...
      <div class="tags_group">
        ...
        <template v-if="show_image_caption">
          <ValidationProvider tag="div" :class="[ 'group bordered', (show_image_mode) ? '' : 'nmb' ]" name="caption" :rules="{ required: false }" :vid="`image_form_${unique}.file_caption[]`" v-slot="{ errors }">
            <label :for="`image_caption${unique}`">Caption <b>(OPTIONAL)</b></label>
      ...
      checkRules () {
        if (this.required) {
          if (this.dimension.imageWidth == 0) {
            ...
            this.file_rules = { required: (this.dataImage.id) ? false : true, image: true, ext: ['jpeg', 'jpg', 'png', 'webp', 'svg'], size: 20000, dimensions: [this.dimension.imageWidth, this.dimension.imageHeight] }
          }
        } else {
          if (this.dimension.imageWidth == 0) {
            this.file_rules = { image: true, ext: ['jpeg', 'jpg', 'png', 'webp', 'svg'], size: 20000 }
          } else {
            this.file_rules = { image: true, ext: ['jpeg', 'jpg', 'png', 'webp', 'svg'], size: 20000, dimensions: [this.dimension.imageWidth, this.dimension.imageHeight] }
          }
        }
