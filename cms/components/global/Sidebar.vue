  ...
  <div id="sidebar" :class="[ (has_toggled) ? 'floating' : '', 'hovered' ]" @focus="enter()" @mouseenter="enter()" @mouseleave="leave()">
    <div class="top">
      ...
      <!-- <div class="toggle pointer" @click="toggleNav()">
        <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24" fill="none" stroke="#E12328" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="fill">
            ...
            <div class="sub" v-if="data.subs">
              <ul class="sub_list">
                <template v-for="(sub, key) in data.subs">
                  <li class="sub_item" :key="key" v-if="checkPermission(sub.unique)">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toggleNav () {
        this.$store.commit('global/sidebar/checkToggler', { status: (this.has_toggled) ? false : true })
      },
      checkHeightViewport () {
