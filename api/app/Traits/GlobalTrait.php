...
trait GlobalTrait
{
    ...
    /**
     * GlobalTrait getWebAds
     * @param string $slug
     * @param Request $request
     * @return object/array
     */
    public function getWebAds ($slug, $request = null)
    {
        if ($slug != 'global') {
            $record['modules'] = AdsInhousePivot::whereHas('adsInhouse', function ($query) use ($slug) {
                $query->where([
                    'slug' => $slug,
                    'global' => 0
                ]);
            ...
            ->with([
                'adsInhouse' => function ($query) {
                    $query->select('id', 'slug', 'global');
                },
                'adsInhouseModule' => function ($query) {
                    $query->select('id', 'slug', 'sticky');
                },
                'adsInhouseClientActive' => function ($query) {
                    $query->select('id', 'ads_inhouse_pivot_id', 'link', 'slider', 'video', 'video_link', 'youtube_link')
                    ->where(function ($query) {
                        $query->where('status', 1);
                    })
                    ->with('images', function ($query) {
                        $query->select('id', 'parent_id', 'name', 'category', 'title', 'alt', 'path', 'path_resized');
                    })
                    ->orderBy('start_date');
                }
            ])
        ...
        if ($slug == 'global') {
            $record['global'] = AdsInhousePivot::whereHas('adsInhouse', function ($query) {
                $query->where([
                    'slug' => 'global',
                    'global' => 1
                ]);
            })
            ->whereHas('adsInhouseModule', function ($query) use ($request) {
                $query->where('slug', $request->a);
            })
            ->with([
                'adsInhouse' => function ($query) {
                    $query->select('id', 'slug', 'global');
                },
                'adsInhouseModule' => function ($query) use ($request) {
                    $query->select('id', 'slug', 'sticky');
                },
                'adsInhouseClientActive' => function ($query) {
                    $query->select('id', 'ads_inhouse_pivot_id', 'link', 'slider', 'video', 'video_link', 'youtube_link')
        ...
        $arrayIndexes = ['modules', 'global'];

        for ($i = 0; $i < count($arrayIndexes); $i++) {
            if (isset($record[$arrayIndexes[$i]])) {
                foreach ($record[$arrayIndexes[$i]] as $key => $module) {
                    unset($module->id);
                    unset($module->ads_inhouse_id);
                    unset($module->ads_inhouse_module_id);
                    if ($module->adsInhouse) {
                        unset($module->adsInhouse->id);
                    }
                    if ($module->adsInhouseModule) {
                        unset($module->adsInhouseModule->id);
                    }
                    if ($module->adsInhouseClientActive) {
                        unset($module->adsInhouseClientActive->id);
                        if (count($module->adsInhouseClientActive->images) > 0) {
                            for ($j = 0; $j < count($module->adsInhouseClientActive->images); $j++) { 
                                unset($module->adsInhouseClientActive->images[$j]->id);
                                unset($module->adsInhouseClientActive->images[$j]->parent_id);
                            }
                        }
                    }
                }
    ...
    public function contentAllocation ($record, $request, $type, $user)
    {
        $allocation = ContentAllocation::where('parent_id', $record->id)->first();
        if (!$allocation) {
            ContentAllocation::create([
                'parent_id' => $record->id,
                'approver_id' => ($user->role->type == 'editor') ? null : $user->id,
                'editor_id' => ($user->role->type == 'editor') ? $user->id : null,
                'type' => $type,
                'status' => ($user->role->type == 'editor') ? 'pending' : 'approved',
                'enabled' => ($user->role->type == 'editor') ? 0 : $request->enabled
            ]);
        } else {
            $allocation->update([
                'parent_id' => $record->id,
                ...
                'editor_id' => ($user->role->type == 'editor') ? $user->id : null,
                'type' => $type,
                'status' => ($user->role->type == 'editor') ? ($request->preview) ? $allocation->status : 'processing' : 'approved',
                'enabled' => ($user->role->type == 'editor') ? 0 : $request->enabled
            ]);
        }
    }

    public function metadata ($record, $request)
    {
    ...
    function validateYoutubeLink ($link) {
        ...
        $response = Http::get("https://www.googleapis.com/youtube/v3/videos?part=snippet&id=$youtubeId&key=AIzaSyAf4W3qK-BTYNxPmndbLDWKVVimnN1XStk");
